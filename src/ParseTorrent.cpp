...
char ParseTorrent::handshakeMessage[28] = { 0x13, 0x42, 0x69, 0x74, 0x54, 0x6f,
		0x72, 0x72, 0x65, 0x6E, 0x74, 0x20, 0x50, 0x72, 0x6F, 0x74, 0x6F, 0x63,
		0x6F, 0x6c, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30 };
...
bt_info_t ParseTorrent::getInfoStructure(map<string, string> m) {
	...
	for (it = m.begin(); it != m.end(); it++) {
		...
		for (int i = 0; i < btStructure.num_pieces; i++) {
			...
			for (int j = 0; j < 20; j++) {
				printf("%02x", intermediate[j]);
			}
cout << endl;
			tryit[i] = intermediate[i];
			for (int j = 0; j < 20; j++) {
				printf("%02x", tryit[j]);
		 ...
		 */ //btStructure.piece_hashes = intermediate;
		  //delete[] p;
		  //delete[] intermediate;
		return btStructure;
...
void ParseTorrent::getHandshakeMessage(unsigned char * hash, bt_args_t * btargs,
		char* hashval) {
